...
class CodeHighlighter extends PolymerElement {
  ...
  connectedCallback() {
    ...
    function processData(data) {
      console.log(data); // alert(data.innerHTML);
      // var trimmedData = (data.innerHTML).trim();

      var trimmedData = data.innerHTML; // alert(trimmedData);

      var formattedString;

      if (data.lang == 'java') {
        console.log('java lang styling needed!!'); //parse inner HTML for keywords and change coloring scheme
        ...
        formattedString = formattedString.replace(/private/g, "<span class='keyword'>private</span>"); // alert(formattedString);
      ...
      data.innerHTML = "<style>pre.vrPre{background-color: gainsboro;overflow-x: auto;}.keyword{color: red;}.quotes{color: blue;}@media (max-width: 500px){#vrContent{font-size:11px;}}@media (max-width: 400px){#vrContent{font-size:10px;}}</style>".concat(formattedString); //   data.innerHTML = formattedString;

      console.log(data.innerHTML); // alert(data.innerHTML);
    }
  }
